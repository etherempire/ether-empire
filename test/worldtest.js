...
contract("World and Trading", async (accounts) => {
    ...
    //     await contract.buildWall(0,2,100, {from: accounts[0]});
    //     let yield = await contract.yieldValue(0,0);
    //     assert.equal(yield/2**32, 100);
    // });

    it("land value modifier is working:", async() => {    
        await contract.buildFarm(0,0,100, {from: accounts[0]});
        await contract.buildFarm(0,1,100, {from: accounts[1]});
        await contract.buildWall(1,1,100, {from: accounts[0]});
        let yield = await contract.yieldValue(0,0);
